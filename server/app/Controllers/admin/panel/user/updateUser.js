	...
	if (!!data && !!data.id && !!data.data) {
		...
		if (!!Object.entries(update).length) {
			UserInfo.findOne({ 'id': data.id }, function (err, check) {
				if (check) {
					console.log("check", check.red)

					let oldRed = check.red.toNumber()
					let temp = Helper.getOnlyNumberInString(update['red'])
					console.log("RedBefore", temp)
					let kk = Number(oldRed) + Number(temp)
					update['red'] = kk;
					console.log("RedAfter", update['red'], temp)
