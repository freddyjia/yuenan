				...
				if (this.auth == false && !!message.authentication) {
					authenticate(this, message.authentication, function(err, success){
						if (success) {
							...
							if(!!userInfo[message.authentication.username]){
								
								let info = userInfo[message.authentication.username];
								if(info.timeSpan < timeSpan){
									info.connection.close(4500);
									info.connection = this
								}
							}else{
								userInfo[message.authentication.username] = {
									timeSpan :timeSpan,
									connection:this
								}
							}
