			...
			Promise.all(list.map(function (obj) {
				let action = new Promise((resolve, reject) => {
					TaiXiu_User.findOne({ uid: obj.uid }, function (error, data) {
						console.log("结果", obj)
						let bet_thua = obj.bet;//输的钱
						let bet = obj.win ? obj.tralai + obj.bet : bet_thua;
						let update = {};
		...
		TXCuoc.find({ phien: game_id }, null, { sort: { '_id': -1 } }, function (err, list) {
			if (list.length) {
				list.forEach(function (objL) {
					if (objL.taixiu === true && objL.red === true && objL.select === true) {           // Tổng Red Tài 红色小
						TaiXiu_red_tong_tai += objL.bet;
					} else if (objL.taixiu === true && objL.red === true && objL.select === false) {  // Tổng Red Xỉu 红色大
						TaiXiu_red_tong_xiu += objL.bet;
					} else if (objL.taixiu === true && objL.red === false && objL.select === true) {  // Tổng Xu Tài 蓝色小
						TaiXiu_xu_tong_tai += objL.bet;
					} else if (objL.taixiu === true && objL.red === false && objL.select === false) { // Tổng Xu Xỉu 蓝色大
						TaiXiu_xu_tong_xiu += objL.bet;
					} else if (objL.taixiu === false && objL.red === true && objL.select === true) {  // Tổng Red Chẵn 红色小总
						ChanLe_red_tong_chan += objL.bet;
					} else if (objL.taixiu === false && objL.red === true && objL.select === false) {  // Tổng Red Lẻ 红色大总
						ChanLe_red_tong_le += objL.bet;
					} else if (objL.taixiu === false && objL.red === false && objL.select === true) {  // Tổng xu Chẵn 蓝色小总
						ChanLe_xu_tong_chan += objL.bet;
					} else if (objL.taixiu === false && objL.red === false && objL.select === false) { // Tổng xu Lẻ蓝色大总 
						ChanLe_xu_tong_le += objL.bet;
					}
				});
				let TaiXiu_tong_red_lech = Math.abs(TaiXiu_red_tong_tai - TaiXiu_red_tong_xiu);//红色左边下注多
				let TaiXiu_tong_xu_lech = Math.abs(TaiXiu_xu_tong_tai - TaiXiu_xu_tong_xiu);//蓝色左边下注多
				...
				let TaiXiu_red_lech_tai = TaiXiu_red_tong_tai > TaiXiu_red_tong_xiu ? true : false;//红色
				let TaiXiu_xu_lech_tai = TaiXiu_xu_tong_tai > TaiXiu_xu_tong_xiu ? true : false; //蓝色
				let ChanLe_red_lech_chan = ChanLe_red_tong_chan > ChanLe_red_tong_le ? true : false;//单双
				let ChanLe_xu_lech_chan = ChanLe_xu_tong_chan > ChanLe_xu_tong_le ? true : false;//单双
				...
				console.log("TaiXiu_tong_red_lech", TaiXiu_tong_red_lech)
				console.log("TaiXiu_tong_xu_lech", TaiXiu_tong_xu_lech)
				console.log("ChanLe_tong_red_lech", ChanLe_tong_red_lech)
				console.log("ChanLe_tong_xu_lech", ChanLe_tong_xu_lech)

				console.log("TaiXiu_red_lech_tai", TaiXiu_red_lech_tai)
				console.log("TaiXiu_xu_lech_tai", TaiXiu_xu_lech_tai)
				console.log("ChanLe_red_lech_chan", ChanLe_red_lech_chan)
				console.log("ChanLe_xu_lech_chan", ChanLe_xu_lech_chan)

				Promise.all(list.map(function (obj) {

					//	console.log("dice", dice > 10, TaiXiu_red_lech_tai, TaiXiu_tong_red_lech)
					let oneUpdate = {};
					if (obj.taixiu === true && obj.red === true && obj.select === true) {           // Tổng Red Tài 下注大
						...
						if (TaiXiu_red_lech_tai && TaiXiu_tong_red_lech > 0) {
							if (TaiXiu_tong_red_lech >= obj.bet) {
								...
								obj.save();
								if (win) {
									let betwinP = truChietKhau(obj.bet, 2);
									let total = obj.bet + truChietKhau(obj.bet, 2);
									UserInfo.updateOne({ id: obj.uid }, { $inc: { red: total } }).exec();
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: true, select: true, red: true }, { $set: { win: true }, $inc: { tralai: betwinP } }).exec();
								} else {
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: true, select: true, red: true }, { $set: { win: false }, $inc: { tralai: obj.bet } }).exec();
								}
								...
								if (win) {
									console.log("user", obj.uid, "左边大——下注大_赢")
									...
									obj.save();
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: true, select: true, red: true }, { $set: { win: true }, $inc: { tralai: obj.tralai, betwin: betwinP } }).exec();
								} else {
									console.log("user", obj.uid, "左边大——下注大_输")
							...
							if (win) {
								...
								obj.tralai = betwin;
								obj.save();
								console.log("user", obj.uid, " 右边大——下注大_赢")
								...
								return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: true, select: true, red: true }, { $set: { win: true }, $inc: { tralai: obj.tralai, betwin: betwin } }).exec();
							} else {
								obj.thanhtoan = true;
								obj.save();
								console.log("user", obj.uid, "右边大——下注大_输")
					...
					} else if (obj.taixiu === true && obj.red === true && obj.select === false) {  // Tổng Red Xỉu  下注小
						...
						if (!TaiXiu_red_lech_tai && TaiXiu_tong_red_lech > 0) {
							if (TaiXiu_tong_red_lech >= obj.bet) {
								...
								if (win) {
									...
									let total = obj.bet + truChietKhau(obj.bet, 2);
									UserInfo.updateOne({ id: obj.uid }, { $inc: { red: total } }).exec();
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: true, select: false, red: true }, { $set: { win: true }, $inc: { tralai: betwinP } }).exec();
								} else {
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: true, select: false, red: true }, { $set: { win: false }, $inc: { tralai: obj.bet } }).exec();
								}
								...
								if (win) {
									// Thắng nhưng bị trừ tiền trả lại
									// cộng tiền thắng
									console.log("user", obj.uid, "左边大——下注小_赢")
									betwinP = truChietKhau(betPlay, 2);
									obj.betwin = betwinP;
									obj.tralai = betwinP;
									let redUpdate = obj.bet + betwinP;
									...
									TaiXiu_User.updateOne({ uid: obj.uid }, { $inc: { tWinRed: betwinP, tRedPlay: betPlay } }).exec();
									obj.save();
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: true, select: false, red: true }, { $set: { win: true }, $inc: { tralai: obj.tralai, betwin: betwinP } }).exec();
								} else {
									console.log("user", obj.uid, "左边大——下注小_输")
									UserInfo.updateOne({ id: obj.uid }, { $inc: { red: obj.tralai, redPlay: betPlay, redLost: betPlay } }).exec();
									TaiXiu_User.updateOne({ uid: obj.uid }, { $inc: { tLostRed: betPlay, tRedPlay: betPlay } }).exec();
								}
							...
							if (win) {
								...
								obj.tralai = betwin;
								obj.save();
								console.log("user", obj.uid, "右边大——下注小_赢")
								let redUpdate = obj.bet + betwin;
								UserInfo.updateOne({ id: obj.uid }, { $inc: { red: redUpdate, redWin: betwin, redPlay: obj.bet } }).exec();
								TaiXiu_User.updateOne({ uid: obj.uid }, { $inc: { tWinRed: betwin, tRedPlay: obj.bet } }).exec();
								return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: true, select: false, red: true }, { $set: { win: true }, $inc: { tralai: obj.tralai, betwin: betwin } }).exec();
							} else {
								obj.thanhtoan = true;
								obj.save();
								console.log("user", obj.uid, "右边大——下注小_输")
								UserInfo.updateOne({ id: obj.uid }, { $inc: { redLost: obj.bet, redPlay: obj.bet } }).exec();
								TaiXiu_User.updateOne({ uid: obj.uid }, { $inc: { tLostRed: obj.bet, tRedPlay: obj.bet } }).exec();
							}
					...
					} else if (obj.taixiu === false && obj.red === true && obj.select === true) {  // Tổng Red Chẵn 下注双

						let win = false;
						if ((dice % 2) > 0) win = false;
						else win = true;
						console.log("下注双", dice % 2, win)
						if (ChanLe_red_lech_chan && ChanLe_tong_red_lech > 0) {
							if (ChanLe_tong_red_lech >= obj.bet) {
								...
								obj.save();
								if (win) {
									console.log("下注双1", win)
									let betwinP = truChietKhau(obj.bet, 2);
									let total = obj.bet + truChietKhau(obj.bet, 2);
									UserInfo.updateOne({ id: obj.uid }, { $inc: { red: total } }).exec();
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: false, select: true, red: true }, { $set: { win: true }, $inc: { tralai: betwinP } }).exec();
								} else {
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: false, select: true, red: true }, { $set: { win: false }, $inc: { tralai: obj.bet } }).exec();
								}
							} else {
								// Trả lại 1 phần
								let betPlay = obj.bet;
								...
								if (win) {
									...
									obj.save();
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: false, select: true, red: true }, { $set: { win: true }, $inc: { tralai: obj.tralai, betwin: betwinP } }).exec();
								} else {
							...
							if (win) {
								...
								return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: false, select: true, red: true }, { $set: { win: true }, $inc: { tralai: obj.tralai, betwin: betwin } }).exec();
							} else {
								obj.thanhtoan = true;
								obj.save();
					...
					} else if (obj.taixiu === false && obj.red === true && obj.select === false) {  // Tổng Red Lẻ 下注单
						//	let win = dice % 2 ? true : false;

						let win = false;
						if ((dice % 2) > 0) win = true;
						else win = false;
						console.log("下注单", dice % 2, win)
						if (!ChanLe_red_lech_chan && ChanLe_tong_red_lech > 0) {
							if (ChanLe_tong_red_lech >= obj.bet) {
								// Trả lại hoàn toàn
								ChanLe_tong_red_lech -= obj.bet
								...
								if (win) {
									...
									let total = obj.bet + truChietKhau(obj.bet, 2);
									UserInfo.updateOne({ id: obj.uid }, { $inc: { red: total } }).exec();
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: false, select: false, red: true }, { $set: { win: true }, $inc: { tralai: betwinP } }).exec();
								} else {
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: false, select: false, red: true }, { $set: { win: false }, $inc: { tralai: obj.bet } }).exec();
								}
							} else {
								// Trả lại 1 phần
								let betPlay = obj.bet;
								...
								if (win) {
									...
									TaiXiu_User.updateOne({ uid: obj.uid }, { $inc: { cWinRed: betwinP, cRedPlay: betPlay } }).exec();
									obj.save();
									return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: false, select: false, red: true }, { $set: { win: true }, $inc: { tralai: obj.tralai, betwin: betwinP } }).exec();
								} else {
									UserInfo.updateOne({ id: obj.uid }, { $inc: { red: obj.tralai, redLost: betPlay, redPlay: betPlay } }).exec();
									TaiXiu_User.updateOne({ uid: obj.uid }, { $inc: { cLostRed: betPlay, cRedPlay: betPlay } }).exec();
								}
							...
							if (win) {
								...
								UserInfo.updateOne({ id: obj.uid }, { $inc: { red: redUpdate, redWin: betwin, redPlay: obj.bet } }).exec();
								TaiXiu_User.updateOne({ uid: obj.uid }, { $inc: { cWinRed: betwin, cRedPlay: obj.bet } }).exec();
								return TXCuocOne.updateOne({ uid: obj.uid, phien: game_id, taixiu: false, select: false, red: true }, { $set: { win: true }, $inc: { tralai: obj.tralai, betwin: betwin } }).exec();
							} else {
								obj.thanhtoan = true;
								obj.save();
