...
let first = function (client) {
	console.log("登录")
	UserInfo.findOne({ id: client.UID }, 'name lastVip redPlay red xu ketSat UID cmt email security joinedOn lastLogin', function (err, user) {
		if (!!user) {
			user = user._doc;
			...
			User.updateOne({ '_id': client.UID }, { $set: { 'local.lastLogin': new Date() } }).exec();
			delete user._id;
			delete user.redPlay;
			delete user.lastVip;
			...
			Phone.findOne({ uid: client.UID }, {}, function (err2, dataP) {
				...
				Kefu.findOne({}, {}, function (err, res) {
					//	console.log("myDocument", res);
					let data = {
						Authorized: true,
						user: user,
						zalo: res.zalo,
						facebook: res.facebook,
						telegrem: res.telegrem,
					};
						...
						if (testBase) {
							...
							UserInfo.findOne({ 'name': name }, 'name', function (err, check) {
								if (!!check) {
									...
									try {
										UserInfo.create({ 'id': client.UID, 'name': name, 'joinedOn': new Date(), 'lastLogin': new Date() }, function (errC, user) {
											if (!!errC) {
