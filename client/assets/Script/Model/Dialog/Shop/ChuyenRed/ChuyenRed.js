...
cc.Class({
	...
	onChuyenClick: function () {
		if (!cc.RedT.code || ((new Date() - Date.parse(this.date)) / 1000) > 180) {
			console.log("获取otp")
			this.onClickOTP();
		} else {
			console.log("直接获取")
			...
			} else if (helper.isEmpty(cc.RedT.code)) {
			...
			if (error)
				...
				var data = { name: this.nickname.string, red: helper.getOnlyNumberInString(this.red.string), otp: cc.RedT.code };
