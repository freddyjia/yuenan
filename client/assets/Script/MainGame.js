...
var copyTool = require('./Util/ClipboardJS');
...
cc.Class({
	...
	onLoad: function () {
		...
		if (void 0 === cc.RedT) {
			...
			cc.RedT.mainNode = this;
			cc.RedT.init();
			cc.RedT.audio = this.PrefabT[0].data.getComponent('MainAudio');
		}
		//	console.log("是否登录", cc.RedT, cc.RedT.mainNode)
		// Connect Server
		cc.RedT.reconnect();
		...
		//cc.log("判断是否登录", cc.RedT.userDataBasic)
		if (userData && userData.username && userData.password) {
			//	cc.log("直接登录", cc.RedT.IS_LOGIN, userData.username, userData.password);
			if (!cc.RedT.IS_LOGIN) {
				cc.RedT.userDataBasic = { username: userData.username, password: userData.password };
				cc.RedT.inGame.auth({ authentication: { username: userData.username, password: userData.password } });
			}
		}
	...
	onData: function (data) {
		...
		if (void 0 !== data.Authorized) {
			...
			cc.log(data.zalo, data.facebook, data.telegrem)
			cc.RedT.kefu = data;
		}
		if (void 0 !== data.user) {

			localStorage.setItem("userData", JSON.stringify(cc.RedT.userDataBasic))
	...
	updateRed: function updateRed(win) {
		if (win < 0) return;
		//	console.log("miniRed", win, "sss", cc.RedT.user.red);
		var red = Number(cc.RedT.user.red) + Number(win);
		//	console.log("miniRedAfter", red);
		if (!this.header) {
			this.header = cc.find("Canvas").getChildByName("header").getComponent("Header");
		}
		this.header.updateRed(helper.numberWithCommas(red));
	},
	AllReset: function () {
		this.loading.active = false;
	...
	showKefu: function (Zalo, face, tel) {
		console.log("kefu", cc.RedT.kefu)
		if (cc.RedT.kefu.zalo) {
			this.avatar.getChildByName("Zalo").getChildByName("text").getComponent(cc.Label).string = cc.RedT.kefu.zalo;
			this.avatar.getChildByName("Facebook").getChildByName("text").getComponent(cc.Label).string = cc.RedT.kefu.facebook;
			this.avatar.getChildByName("telegrem").getChildByName("text").getComponent(cc.Label).string = cc.RedT.kefu.telegrem;
		}
		if (this.avatar.getChildByName("Zalo").active == true) {
			this.avatar.getChildByName("Zalo").active = false;
			this.avatar.getChildByName("Facebook").active = false;
			this.avatar.getChildByName("telegrem").active = false;
		}

		else {
			this.avatar.getChildByName("Zalo").active = true;
			this.avatar.getChildByName("Facebook").active = true;
			this.avatar.getChildByName("telegrem").active = true;
		}

	},
	copyText: function (event, customEventData) {
		cc.log("复制", customEventData)
		if (customEventData == 1) {
			copyTool.Copy(this.avatar.getChildByName("Zalo").getChildByName("text").getComponent(cc.Label).string)
		} else if (customEventData == 2) {
			copyTool.Copy(this.avatar.getChildByName("Facebook").getChildByName("text").getComponent(cc.Label).string)
		} else if (customEventData == 3)
			copyTool.Copy(this.avatar.getChildByName("telegrem").getChildByName("text").getComponent(cc.Label).string)
	},
	/**
